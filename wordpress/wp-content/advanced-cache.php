...
// WP-Optimize advanced-cache.php (written by version: 4.2.3) (homeurl: http://localhost:8280/) (abspath: /var/www/html/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/var/www/html/wp-content/plugins/wp-optimize-premium/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://localhost:8280/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/var/www/html/wp-content/plugins/wp-optimize-premium/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-localhost-port8280.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-localhost-port8280.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-localhost-port8280.php');
}
