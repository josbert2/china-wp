 ...
 */
add_filter('pre_http_request', function($preempt, $parsed_args, $url) { if ($parsed_args['method'] === 'POST' && strpos($url, 'https://teamupdraft.com/plugin-info/') !== false) { $response_array = [ "version" => 1, "code" => "OK", "data" => [ "plugin_info" => [ "name" => "WP Optimize Premium", "slug" => "wp-optimize-premium", "author" => "teamupdraft.com", "homepage" => "https://teamupdraft.com", "sections" => [ "description" => "Optimize your WordPress database.", ], ] ] ]; $response_body = json_encode($response_array); return [ 'headers' => [], 'body' => $response_body, 'response' => [ 'code' => 200, 'message' => 'OK' ], ]; } return $preempt; }, 10, 3);
