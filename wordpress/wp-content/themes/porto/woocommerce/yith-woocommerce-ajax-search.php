...
<form role="search" method="get" id="yith-ajaxsearchform" action="<?php echo esc_url( home_url( '/' ) ); ?>" class="yith-ajaxsearchform-container <?php echo esc_attr( $container_class ); ?> searchform<?php echo ! $show_cats ? '' : ' searchform-cats'; ?>">
	<fieldset>
		...
		?>
		<span class="button-wrap"><button class="btn" id="yith-searchsubmit" aria-label="<?php esc_attr_e( 'Search', 'woocommerce' ); ?>" title="<?php esc_attr_e( 'Search', 'woocommerce' ); ?>" type="submit"><i class="fas fa-search"></i></button></span>
