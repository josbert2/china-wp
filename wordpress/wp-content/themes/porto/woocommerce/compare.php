...
<h1>
	...
	if ( ! $is_iframe ) :
		?>
		<a role="button" aria-labelledby="<?php esc_attr_e( 'Close the compare popup', 'porto' ); ?>" class="close" href="#"><?php esc_html_e( 'Close window [X]', 'yith-woocommerce-compare' ); ?></a><?php endif; ?>
</h1>
		...
		<tr class="remove">
			...
			foreach ( $products as $product_id => $product ) :
				...
				<td class="<?php echo esc_attr( $product_class ); ?>">
					<a role="button" href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product_id ) ); ?>" data-product_id="<?php echo esc_attr( $product_id ); ?>"><?php esc_html_e( 'Remove', 'yith-woocommerce-compare' ); ?> <i class="close-icon"></i></a>
				</td>
				<?php
				++$index;
			endforeach;
