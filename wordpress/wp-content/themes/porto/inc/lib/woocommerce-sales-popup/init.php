		...
		if ( ! empty( $_POST['load_first'] ) ) {
			...
			if ( $result ) {
				foreach ( $result as $item ) {
					...
					$selected_img = wp_get_attachment_image_src( $product->get_image_id(), 'woocommerce_gallery_thumbnail' );
					$products[]   = array(
						'id'     => esc_html( $item->product_id ),
						'title'  => esc_html( $product->get_title() ),
						'link'   => esc_url( $product->get_permalink() ),
						'image'  => esc_js( ! empty( $selected_img[0] ) ? $selected_img[0] : wc_placeholder_img_src() ),
