...
<?php echo ( ! empty( $show_thumbnail ) ? '<div class="thumbnail-wrapper">' : '' ); ?>
	<div class="post-image<?php echo 1 == $image_count ? ' single' : ''; ?>">
		<div class="post-slideshow porto-carousel owl-carousel has-ccols ccols-1 <?php echo esc_attr( $el_class ); ?>" data-plugin-options='<?php echo json_encode( $carousel_options ); ?>'>
			...
			foreach ( $featured_images as $featured_image ) {
				...
				<div>
			<?php else : ?>
				<a href="<?php echo esc_url( apply_filters( 'the_permalink', get_permalink() ) ); ?>" aria-label="<?php esc_attr_e( 'post image', 'porto' ); ?>">
			<?php endif; ?>
	...
	<?php if ( ! empty( $show_thumbnail ) ) : ?>
			...
			if ( $image_count >= 1 ) :
				...
				?>
				<div class="porto-carousel thumbnail-gallery owl-carousel show-nav-hover has-ccols has-ccols-spacing ccols-<?php echo porto_filter_output( $slider_thumbs_count ); ?>" data-plugin-options="<?php echo esc_attr( json_encode( $options ) ); ?>">
				<?php
				foreach ( $featured_images as $featured_image ) {
